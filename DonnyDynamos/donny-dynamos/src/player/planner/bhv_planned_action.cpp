...
#include "xgboostalg.h" // for predicting shot xg
    ...
    case CooperativeAction::Shoot:
        {
            ...
            // NEW SHOOTING LOGIC
            std::cout << "ABOUT TO SHOOT - CHECKING XG";
            
            // get xg at curent location
            Shot current_shot;
            current_shot.location.x = wm.ball().pos().x;
            current_shot.location.y = wm.ball().pos().y;
            current_shot.xg = 0.0;
            predictShotxg(current_shot);
            
            Shot best_shot = current_shot;
            
            // if above threshold - shoot
            double xg_threshold = 0.2;
            if (current_shot.xg > xg_threshold) {
                if (Body_ForceShoot().execute(agent)) {
                    agent->setNeckAction(new Neck_TurnToGoalieOrScan(2));
                    return true;
                }
            } else {
                // if not - scan surrounding area for higher xg areas
                for (double dx = -2.0; dx <= 2.0; dx += 1.0) {
                    for (double dy = -2.0; dy <= 2.0; dy += 1.0) {  
                        Point test_pos;
                        test_pos.x = current_shot.location.x + dx;
                        test_pos.y = current_shot.location.y + dy;
                        
                        if (test_pos.x < SP.pitchLength() && test_pos.y < SP.pitchWidth()) continue;

                        Shot test_shot;
                        test_shot.location.x = test_pos.x;
                        test_shot.location.y = test_pos.y;
                        test_shot.xg = 0.0;
                        predictShotxg(test_shot);

                        if (test_shot.xg > best_shot.xg) {
                            best_shot = test_shot;
                        }
                    }
                }
            ...
            // if nearby areas are greater than current location - continue dribbling
            if (best_shot.location.y != current_shot.location.y && best_shot.location.x != current_shot.location.x && best_shot.xg > current_shot.xg){
                std::cout << "DRIBBLE TO BETTER LOCATION";
                break;
            } else {
                // if current location is higher than all surrounding areas just shoot
                std::cout << "CURRENT LOCATION IS BEST - SHOOTING";
                if (Body_ForceShoot().execute(agent)) {
                    agent->setNeckAction(new Neck_TurnToGoalieOrScan(2));
                    return true;
                }
