...
const std::string Strategy::OFFENSE_FORMATION_OVERLAPPING_CONF = "offense-formation-overlappingfb.conf";
const std::string Strategy::OFFENSE_FORMATION_FALSENINE_CONF = "offense-formation-falseNine.conf";
const std::string Strategy::DEFENSE_FORMATION_442BLOCK_CONF = "defense-formation-442block.conf";
const std::string Strategy::OFFENSE_FORMATION_INVERTEDFB_CONF = "offense-formation-invertedfb.conf";
const std::string Strategy::DEFENSE_FORMATION_433PRESS_CONF = "defense-formation-433press.conf";
      ...
      M_positions( 11 )
{
    ...
    M_role_factory[RoleOverlapFB::name()] = &RoleOverlapFB::create;
    M_role_factory[RoleFalseNine::name()] = &RoleFalseNine::create;
    M_role_factory[RoleInvertedFB::name()] = &RoleInvertedFB::create;
...
Strategy::read( const std::string & formation_dir )
{
    ...
    M_offense_formation_overlapping = createFormation(configpath + OFFENSE_FORMATION_OVERLAPPING_CONF );
    if (!M_offense_formation_overlapping)
    {
        std::cerr << "Failed to read offense overlapping fb formation" << std::endl;
        return false;
    }

    M_offense_formation_falsenine = createFormation(configpath + OFFENSE_FORMATION_FALSENINE_CONF);
    if (!M_offense_formation_falsenine)
    {
        std::cerr << "Failed to read offense false nine formation" << std::endl;
        return false;
    }

    M_defense_formation_442block = createFormation(configpath + DEFENSE_FORMATION_442BLOCK_CONF);
    if (!M_defense_formation_442block)
    {
        std::cerr << "Failed to read defense formation 442 block" << std::endl;
        return false;
    }

    M_offense_formation_invertedFB = createFormation(configpath + OFFENSE_FORMATION_INVERTEDFB_CONF);
    if (!M_offense_formation_invertedFB)
    {
        std::cerr << "Failed to read offense inverted fb formation" << std::endl;
        return false;
    }

    M_defense_formation_433press = createFormation(configpath + DEFENSE_FORMATION_433PRESS_CONF);
    if (!M_defense_formation_433press)
    {
        std::cerr << "Failed to read defense formation 433 press" << std::endl;
        return false;
    }
...
Strategy::updateSituation( const WorldModel & wm )
{
    ...
    int their_score = (wm.ourSide() == LEFT) ? wm.gameMode().scoreRight() : wm.gameMode().scoreLeft();
    int score_diff = our_score - their_score;

    /*
    * OLD WAY OF DOING SITUATIONS
    * 
    ...
    */
    ...
    /*
    NEW WAY TO DO SITUATIONS
    checks if we're winning or losing THEN if we have the ball or not
    */
    ...
    if (score_diff > 0) {
       // if we are winning
        ...
        {
            dlog.addText(Logger::TEAM,
                __FILE__": Situation Defense WINNING");
            M_current_situation = Defense_Situation_Winning;
            return;
        }
        ...
        {
            dlog.addText(Logger::TEAM,
                __FILE__": Situation Offense WINNING");
            M_current_situation = Offense_Situation_Winning;
            return;
        }
    }
       ...
       // if we are drawing
        ...
        {
            dlog.addText(Logger::TEAM,
                __FILE__": Situation Defense LOSE/DRAW");
            M_current_situation = Defense_Situation_LoseDraw;
            return;
        }
        ...
        {
            dlog.addText(Logger::TEAM,
                __FILE__": Situation Offense DRAW");
            M_current_situation = Offense_Situation_Draw;
            return;
        }
    }
    else {
        ...
        {
            dlog.addText(Logger::TEAM,
                __FILE__": Situation Offense LOSE");
            M_current_situation = Offense_Situation_Lose;
            return;
        }
    }

    dlog.addText(Logger::TEAM, __FILE__": Situation Normal");
    M_current_situation = Normal_Situation;
...
Strategy::getFormation( const WorldModel & wm ) const
{
    ...
    if ( wm.gameMode().type() == GameMode::PlayOn )
    {
        ...
        case Defense_Situation_Winning:
            return M_defense_formation_442block;
        case Offense_Situation_Winning:
            return M_offense_formation_falsenine;
        case Defense_Situation_LoseDraw:
            return M_defense_formation_433press;
        case Offense_Situation_Lose:
            return M_offense_formation_overlapping;
        case Offense_Situation_Draw:
            return M_offense_formation_invertedFB;
        default:
            break;
        }
